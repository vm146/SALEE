...
eventQueue = Queue(maxsize = 30)


def eventDet(frame, timeLast, meanLast, POWER_TRESHHOLD=9):
    """ function for event detection in electricity circuit

        Args:
            frame(numpy.array): frame with measurements
            timeLast(int): "start time" for new interval
            meanLast(float): mean value at start of interval
            POWER_TRESHHOLD: constant of 9 Watt

        Returns:
            updated value for timeLast and meanLast
        """

    sendFlag = False
    if "App" in deviceDict:
        app = deviceDict["App"]["ms"]
        sendFlag = True
    meanNow = 0


    # calculate mean of active power

    if timeLast != round(time.time()):                  # calculate only one mean per sec
        if timeLast % 2 == 0:                           # time interval of 2 sec
            for i in range(1, len(frame), 2):
                meanNow += frame[i][0]                  # frame[i][0] - active power
            meanNow /= len(frame) // 2

            # event detection

            diff = meanNow - meanLast

            if sendFlag:
                if diff * (-1) >= POWER_TRESHHOLD:
                    print("Event detected: Down")
                    dataToSend = "Down|" + str(round(diff, 1)) + "|" + str(time_format_ymdhms(time.time())[:-4])
                    if sendFlag and deviceDict["App"]["running"]:
                        eventQueue.put(dataToSend)
                        app.sendMsg(dataToSend)

                if diff >= POWER_TRESHHOLD:
                    print("Event detected: Up")
                    dataToSend = "Up|" + str(round(diff, 1)) + "|" + str(time_format_ymdhms(time.time())[:-4])
                    if sendFlag and deviceDict["App"]["running"]:
                        eventQueue.put(dataToSend)
                        app.sendMsg(dataToSend)
                ...
                print("mean")
                print(meanNow)
                if eventQueue.qsize() > 0 and app.applianceQueue.qsize() > 0:
                    appliance = app.applianceQueue.get()
                    eve = eventQueue.get()
                    app.labels += [" " + appliance + " : " + eve]
                    # save labels into eventLabels.csv-file
                    with open("eventLabels.csv", "a", newline="") as file:
                        writer = csv.writer(file)
                        writer.writerow([app.labels[len(app.labels) - 1]])

        timeLast = round(time.time())
    return meanLast, timeLast
...
def constructFFMPEGCall(ms, name=None, ts=None, path=None, filename=None, subfolder=None):
    ...
    if path is not None:
        systemCall = systemCall.replace("<path>", path.rstrip(os.path.sep))
    else:
        systemCall = systemCall.replace("<path><sep>", "")
    ...
    if filename is not None:
        ...
        if "unknown" in ms.deviceName:
            print(
                bcolors.WARNING + "Found unknown device, will use name as filename, lets hope names are unique" + bcolors.ENDC)
...
def handleDead(ms, ttime):
    ...
    if deadTime > REGARD_AS_DEAD_TIME:
        ...
        if "App" in deviceDict and deviceDict[ms.deviceName]["running"]:
            app = deviceDict["App"]["ms"]
            app.sendMsg(str(ms.deviceName) + " DEAD")
...
def clientConnected(netClient):
    ...
    """
    global deviceDict                                       # all connected devices are listed here
    ...
    if typeFound:
        ...
        if ms.deviceName in deviceDict:
            # This should stop the existing thread
            deviceDict[ms.deviceName]["running"] = False
...
def handleDevice(device, name):
    ...
    if ms.deviceName is not SmartDevice.APP:
        ...
        ffmpegP = subprocess.Popen(calls[0], shell=True, stdin=subprocess.PIPE, stdout=FNULL, stderr=FNULL,
    ...
    meanLast = 0  # last active power mean-value
    timeLast = 0  # last time-value

    while device["running"] and ms.sampling and not ms.brokenConnection:
        # prevent 100% CPU
        time.sleep(0.1)
        # shown = False
        """if ms.deviceName is SmartDevice.APP:
            print("ms deviceName: ")
            print(ms.frames)"""
        ...
        while len(ms.frames) > 0:
            ...
            meanLast, timeLast = eventDet(ms.frames[0], timeLast, meanLast)
            ...
            if device["storeTime"] and device["samples"] + len(ms.frames[0]) >= device["samplesNext"]:
                ...
                ffmpegP.stdin.write(
                    remFrame.transpose().view(np.float32).reshape(remFrame.shape + (-1,)).flatten().tobytes())
                ...
                callAndFileName = constructFFMPEGCall(ms, path=args.outputFolder,
                                                      ts=startTs if withTS else None,
                                                      subfolder=ms.deviceName if args.subfolder else None)
                ffmpegP = subprocess.Popen(callAndFileName[0], shell=True, stdin=subprocess.PIPE, stdout=FNULL,
                                           stderr=FNULL, preexec_fn=os.setsid)
                device["proc"] = ffmpegP
                ...
                ffmpegP.stdin.write(
                    ms.frames[0].transpose().view(np.float32).reshape(ms.frames[0].shape + (-1,)).flatten().tobytes())
                del ms.frames[0]
...
def selectDevices(active=True):
    ...
    if active:
        deviceList = devGetter.terminalSelectionProcess([{"name": deviceDict[dev]["ms"].name,
                                                          ...
                                                          "type": deviceDict[dev]["ms"].TYPE} for dev in
                                                         deviceDict.keys()
...
def listDevices():
    ...
    for name in names:
        ...
        print(
            "\033[95m{:<14}\033[0m{:<10}{:<16}{:<11}{:<25}".format(name[:14], deviceDict[name]["recover"], ip, str(avg),
                                                                   start))
...
def checkInput():
    ...
    # More convenient than using Ctrl-C
    ...
    # def calcPower(voltage, current, samplingRate, upsamplingMethod=None, LINE_FREQUENCY=50):
    """Calculates active, reactive and apparent power of a sampling stream, given voltage(v), current(i) and the sampling rate

    Args:
        voltage(numpy.array or list): voltage in volt
        current(numpy.array or list): current in miliampere
        samplingRate(int): samplingrate of phase in Hz
        (UpsamplingMethod(str): options: "linear" and "repeat")

    Returns: Active power(p), reactive power(q) and apparent power(s) as numpy.array

    """
    """
    sfos = int(samplingRate/LINE_FREQUENCY)
    numPoints = len(current)
    ...
    # Convert because current is in mA
    s = 0.001 * vrms * irms
...
def initParser():
    ...
    parser.add_argument("--measures_powermeter", type=str,
                        choices=[",".join(measure['keys']) for measure in PowerMeter.AVAILABLE_MEASURES],
    ...
    parser.add_argument("--measures_smartmeter", type=str,
                        choices=[",".join(measure['keys']) for measure in SmartMeter.AVAILABLE_MEASURES],
    ...
    parser.add_argument("-l", "--log_level", type=str, choices=["all", "info", "debug", "warning", "error"],
                        default="info",
...
if __name__ == '__main__':
    ...
    with open("eventLabels.csv", "a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Labels with '#' define auto-labeled events"])
    server = NetServer(port=args.port, address="0.0.0.0", connectedCallback=clientConnected, updateInThread=True,
                       logger=serverLogger)
    server.start()
