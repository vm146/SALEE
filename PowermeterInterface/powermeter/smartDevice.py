...
class SmartDevice(metaclass=ABCMeta):
    ...
    #: Type for device which is an application for Smartphone
    APP = "App"
    ...
    def __init__(self, name=None, deviceName=None, samplingRate=None, 
        ...
        self.labels = []
        self.labelsTwo = []
        #: Queue for appliance names of events
        self.applianceQueue = Queue(maxsize=30)
    ...
    def sendList(self, lst):
        """
        Send a list to the socket device.

        :param lst: list
        :type  lst: str
        """
        try:
            lstToSend = json.dumps(lst).encode('utf-8')
            self.socket.sendall(lstToSend)
    ...
    def systemInfo(self, sysInfoCB=None):
        ...
        :return:    If info has been received already, dict is passed directly.
    ...
    def _receiveSocket(self):
        ...
        if len(ready_to_read) > 0:
            self._readSocket()
            # program allows only sent elements with Prefix "Info:" or "Data:" and suffix "\n"
            # Handle all incoming data
            while len(self.recBuffer) > 0:
                # If prefix has not been found
    ...
    def _handleCommand(self, cmd, di):
        ...
        print(di)                       # TODO: Maybe we can initialize here SmartDevice.App?
        if "type" in di:
            self.TYPE = di["type"]
        if "name" in di:
    ...
    def _handleLine(self, line):
        ...
        # if self.verbose:
        string = string.strip()
        # authentification for app connection
        if string == "App":
            self.name = SmartDevice.APP
            self.deviceName = SmartDevice.APP
            # Do NOT change type. Otherwise you'll get kicked from server
        # handle appliance name data from app client
        if string.find("#") > -1:
            string = string[1:]
            self.applianceQueue.put(string)
