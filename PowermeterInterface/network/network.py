...
class NetServer():
    ...
    def start(self):
        ...
        self.server.listen(5)				# up to 5 clients can connect
        self.server.setblocking(0)			# other processes don't need to wait
    ...
    def update(self):
        ...
        for s in readable:
            if s is self.server:
                ...
                self.clients += [client]						# Add client to Servers client list
                return client
        return None
...
class NetClient():
    ...
    def sendData(self, data, lengthPrefix=False, blocking=False):
        ...
        """
        s_data = b''							# b before string denotes conversion into byte (ASCII)
